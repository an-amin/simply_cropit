...
function simplyCropit(obj={})
{
	try {
		if(obj.image_editor==undefined)
			throw "arg.image_editor is undefined";
	}
	catch(err) {
		console.warn(err);
		return err;
	...
	let _cropit = {};
	_cropit.imgWidth = obj.imgWidth!=undefined ? obj.imgWidth : 500;
	...
	_cropit.hiddenImgData = _cropit.editor.find('.hidden-image-data');
	_cropit.error_container = obj.error_container!=undefined ? _cropit.editor.find(obj.error_container) : false;
	_cropit.updateImgData = function(){
		var imgData = _cropit.editor.cropit('export',{
			quality 	: _cropit.quality, 
	...
	_cropit.editor.cropit('previewSize', { 
		width 	: _cropit.previewWidth, 
		height 	: _cropit.previewHeight 
	});

	_cropit.editor.on('click','.rotate-cw',function(){
		_cropit.editor.cropit('rotateCW');
		_cropit.updateImgData();
	});

	_cropit.editor.on('click','.rotate-ccw',function(){
